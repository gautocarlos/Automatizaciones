# Clase que se utiliza para dar de alta un nuevo documento GEDO del tipo libre#
  ...
  #require './elementosHTML.rb' # Abstracción de comportamiento de los elementos HTML
  ...
  class Documento
    ...
    def initialize(browser)
      #@browser = browser
      self.setElementosHTMLFactory(ElementosHTMLFactory.new(browser))
    ...
    def iniciarProduccionDocumento()
      ...
      posicion = 0
      claseDIV = 'z-toolbarbutton'
      claseBandbox = 'z-bandbox-inp'
      self.getElementosHTMLFactory().getElemento('div').realizarClick(posicion, claseDIV)
      #Espera a que se cargue el popup de inicio de documento
      self.getElementosHTMLFactory().getElemento('input').esperarComponente(posicion, claseBandbox)
      #TIPO DE DOCUMENTO A PRODUCIR
      self.getElementosHTMLFactory().getElemento('input').completarTexto(self.getAcronimo(), posicion, claseBandbox)
...
#    end
    ...
    def importarArchivo()
      #self.getBrowser().file_field(:name => 'file').set(self.getRutaImportado()) # ORIGINAL
      posicion = 0
      nombreElemento = 'file' # tipo de input
      self.getElementosHTMLFactory().getElemento('inputfile').subirArchivoPorNombreElemento(self.getRutaImportado(), posicion, nombreElemento)
    end
    ...
    def firmaDeDocumentoConEmbebidosConCertificado()
      ...
      claseDIV = 'z-window-highlighted-close'
      posicion = 0
      #self.getBrowser().div(:class => claseDIV).click # ORIGINAL
      self.getElementosHTMLFactory().getElemento('div').realizarClick(posicion, claseDIV)
      self.parsearBotonesFirmarDocumento()
    ...
    def copiarCodigoSadeGenerado
      self.getBrowser().execute_script("$('body').append('<div id =\"log\" ></div>'); $('#log').text($($('span').get($('span').size() - 2)).text())") # ORIGINAL      
      # Se loguea el código SADE del documento generado - Falta Getter
      @log.info(self.getBrowser().div(:id => 'log').text()) # ORIGINAL      
    ...
    def parsearBotonesDescargarVolverBuzonTareas()
      ...
      pics = self.getElementosHTMLFactory().getElemento('div').obtenerImagenesPorClase(claseDIV)
      pics.each do |img|
        sp = img.src.split('/')
        pic = sp[sp.length - 1]
        #puts pic
